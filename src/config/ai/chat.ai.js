...
export async function lightChatAi(chatMessages) {
  ...
  if (provider === 'ollama') {
    return await ollamaChat(chatMessages, undefined); // Light models usually don't need tools
  } else if (provider === 'openai') {
    return await openAiChat(chatMessages, undefined); // Light models usually don't need tools
  } else {
